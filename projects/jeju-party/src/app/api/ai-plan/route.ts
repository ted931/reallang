...
const COURSES_SYSTEM_PROMPT = `당신은 제주도 코스 설계 전문 AI입니다. 사용자의 취향에 맞는 최적의 제주 코스 3가지를 만들어주세요.
...
const DETAIL_SYSTEM_PROMPT = `당신은 제주도 여행 전문 AI 플래너입니다. 선택된 코스에 맞는 일자별 상세 일정을 만들어주세요.

## 규칙
1. 반드시 실존하는 제주도 장소만 추천하세요.
2. 동선이 효율적이도록 인접한 장소끼리 묶으세요 (제주시권, 서귀포권, 동부, 서부 등).
3. 각 장소에 예상 비용을 반드시 포함하세요 (원 단위).
4. 이동 시간을 고려하여 너무 빡빡하지 않게 구성하세요.
5. 식사 시간(아침/점심/저녁)을 반드시 포함하세요.
6. 날짜별 테마를 설정하세요 (예: "서쪽 해안 힐링", "동쪽 자연 탐방").
7. 각 장소에 장소 간 이동시간(driveMinutes)을 포함하세요.
...
export async function POST(request: Request) {
  ...
  if (body.action === "courses") {
    const { styles = [], companion = "커플", nights = 2, travelers = 2, prompt = "" } = body;
    ...
    const userMessage = [
      ...
      `인원: ${travelers}명`,
      styles.length ? `여행 스타일: ${styles.join(", ")}` : "",
      "",
      "3가지 서로 다른 성격의 코스 카드를 만들어주세요. 각 코스의 highlights만 포함하고 상세 stops는 포함하지 마세요.",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    try {
      const completion = await openai.chat.completions.create({
        ...
        max_tokens: 1500,
        messages: [
          { role: "system", content: COURSES_SYSTEM_PROMPT },
      ...
      console.error("Course cards generation error:", err);
  ...
  // ── 2단계: 선택된 코스의 일자별 상세 일정 생성 ────────────
  if (body.action === "detail") {
    const { courseId = "A", courseName = "", courseDescription = "", nights = 2, travelers = 2, companion = "커플", styles = [], prompt = "" } = body;

    const userMessage = [
      `선택된 코스: ${courseId}. ${courseName}`,
      courseDescription ? `코스 설명: ${courseDescription}` : "",
      prompt,
      `동행: ${companion}`,
      `숙박: ${nights}박 ${nights + 1}일`,
      `인원: ${travelers}명`,
      styles.length ? `여행 스타일: ${styles.join(", ")}` : "",
      "",
      `위 코스에 맞는 ${nights + 1}일간의 상세 일정을 만들어주세요.`,
    ]
      .filter(Boolean)
      .join("\n");
    ...
    try {
      ...
      return NextResponse.json(
        { error: err.message || "상세 일정 생성 중 오류가 발생했습니다." },
        { status: 500 }
      );
    }
  }
