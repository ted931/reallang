...
<template>
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
    ...
    <div class="mb-8 animate-fade-in">
      <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">학습하기</h1>
      <p class="text-gray-500 mt-2">카테고리별로 영어 패턴과 표현을 학습하세요.</p>
    </div>
        ...
        <Transition name="page">
          <div v-if="expandedCategory === category.id" class="mt-3 space-y-2">
            <RouterLink
              ...
              :key="pattern.id"
              :to="`/learn/${pattern.id}`"
              class="block glass rounded-xl p-4 card-hover-sm group animate-slide-up"
              :class="'stagger-' + (idx + 1)"
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div class="flex-1 min-w-0">
                    <h3 class="font-medium text-gray-900 group-hover:text-indigo-600 transition-colors truncate">{{ pattern.title }}</h3>
                    <p class="text-xs text-gray-400 mt-0.5 truncate">{{ pattern.description }}</p>
                  </div>
                </div>
                <div class="flex items-center gap-2 shrink-0 ml-3">
                  <!-- CEFR badge -->
                  <span :class="['text-[10px] px-2 py-0.5 rounded-full font-bold bg-gradient-to-r', getDifficultyColor(pattern.difficulty)]">
                    {{ getDifficultyLabel(pattern.difficulty) }}
                  </span>
                  <span :class="['text-[10px] px-2 py-0.5 rounded-full font-medium bg-gradient-to-r', getDifficultyColor(pattern.difficulty)]">
                    {{ pattern.difficulty }}
                  </span>
              ...
              <div class="mt-2 text-xs text-gray-400 pl-9">예문 {{ pattern.examples.length }}개</div>
            </RouterLink>
          </div>
        </Transition>
      </div>
